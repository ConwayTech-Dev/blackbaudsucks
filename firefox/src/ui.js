...
(async () => {
  const loadBetweenPages = await browser.storage.sync.get({
    loadBetweenPages: true,
  });
  const fixFavicon = await browser.storage.sync.get({ fixFavicon: true });
  const oldAssignmentCenter = await browser.storage.sync.get({
    oldAssignmentCenter: false,
  });
  const wideUI = await browser.storage.sync.get({ wideUI: true });
